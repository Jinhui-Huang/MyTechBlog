...
- b.http1.1协议：===“交通法规”
  三大特定 :
<br>`单向性` ——   网络之间通信 一定是先由客户端 向服务器发送请求，再由服务器给出响应。
<br>`长链接` ——   一旦客户端与服务器建立连接之后，可以无限次发送请求  获得响应，直到客户端主动断开连接 
<br>`无状态 `——   服务器端 不进行数据的保存。
<br>但是现在网络之间有的数据存储，是有对应的技术：Cookie ，session ，中间件等
- c.html  超文本标签/标记语言
## 2.head 标签里常用的子标签：
